...
import 'dart:io';
import 'package:dbug_vpn/utils/platform_imports.dart' show IronSource, LevelPlayBannerAdView, LevelPlayAdSize, LevelPlayBannerAdViewListener, LevelPlayAdInfo, LevelPlayAdError, IronSourceInitializationListener;
...
// import 'package:ironsource_mediation/ironsource_mediation.dart'; // Using platform_imports instead
...
class AdsHelper {
  ...
  void showInterAd([Future<Null> Function()? param0]) {
    if (adsType.contains("0") && !_adsController.isPremium.value) {
  ...
  static Future<void> loadIronSourceInterstitialAd() async {
    // Note: Updated to use new LevelPlay API - specific implementation may vary
    // await IronSource.loadInterstitial(); // Deprecated
    // The new API requires creating a LevelPlayInterstitialAd instance
    // This is a placeholder - actual implementation depends on your IronSource setup
    debugPrint(
        'IronSource interstitial load - update to LevelPlayInterstitialAd required');
  }

  static Future<void> showIronSourceInterstitialAd() async {
    // Note: Updated to use new LevelPlay API - specific implementation may vary
    // if (await IronSource.isInterstitialReady()) { // Deprecated
    //   IronSource.showInterstitial(); // Deprecated
    // } else {
    //   loadIronSourceInterstitialAd();
    // }
    // The new API requires using LevelPlayInterstitialAd instance
    debugPrint(
        'IronSource interstitial show - update to LevelPlayInterstitialAd required');
    loadIronSourceInterstitialAd();
  }
  ...
  Future<void> loadIronSourceRewardedAd() async {
    // Note: Updated to use new LevelPlay API - specific implementation may vary
    // await IronSource.loadRewardedVideo(); // Deprecated
    // The new API requires creating a LevelPlayRewardedAd instance
    debugPrint(
        'IronSource rewarded load - update to LevelPlayRewardedAd required');
  }

  Future<void> showIronSourceRewardedAd() async {
    // Note: Updated to use new LevelPlay API - specific implementation may vary
    // if (await IronSource.isRewardedVideoAvailable()) { // Deprecated
    //   IronSource.showRewardedVideo(); // Deprecated
    // } else {
    //   await loadIronSourceRewardedAd();
    // }
    // The new API requires using LevelPlayRewardedAd instance
    debugPrint(
        'IronSource rewarded show - update to LevelPlayRewardedAd required');
    await loadIronSourceRewardedAd();
  }
