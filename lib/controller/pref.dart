// ignore_for_file: curly_braces_in_flow_control_structures

import 'dart:convert';

import 'package:hive_flutter/hive_flutter.dart';

import '../model/server_model.dart';
import '../model/vpn.dart';

class Pref {
  static late Box _box;

  static Future<void> initializeHive() async {
    await Hive.initFlutter();
    _box = await Hive.openBox('data');
  }

  //for storing theme data
  static bool get isDarkMode => _box.get('isDarkMode') ?? false;
  static set isDarkMode(bool v) => _box.put('isDarkMode', v);

  //for storing single selected vpn details
  static Vpn get vpn => Vpn.fromJson(jsonDecode(_box.get('vpn') ?? '{}'));
  static set vpn(Vpn v) => _box.put('vpn', jsonEncode(v));

  //serverVpn
  static Server get vpnServer =>
      Server.fromJson(jsonDecode(_box.get('vpnServer') ?? '{}'));
  static set vpnServer(Server v) => _box.put('vpnServer', jsonEncode(v));

  //for storing vpn servers details
  static List<Vpn> get vpnList {
    List<Vpn> temp = [];
    final data = jsonDecode(_box.get('vpnList') ?? '[]');

    for (var i in data) temp.add(Vpn.fromJson(i));

    return temp;
  }

  static set vpnList(List<Vpn> v) => _box.put('vpnList', jsonEncode(v));

  //for storing and retrieving authentication token
  static String? get token => _box.get('authToken');
  static set token(String? value) => _box.put('authToken', value);

  //getToken method for compatibility with SplitTunnelApi
  static Future<String?> getToken() async {
    return token;
  }
}
