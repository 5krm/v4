...
class AuthController extends GetxController {
  ...
  Future<ResponseModel> updateProfile(
    ...
    if (response.statusCode == 200) {
      ...
      responseModel =
          ResponseModel(false, response1model.message, response.statusCode);
    }
    _isLoading = false;
    update();
    return responseModel;
  ...
  Future<Response> updateProfileClient(
    ...
    return await apiClient.postData(
        MyHelper.updateProfile, {"name": name, "phone": phone, "email": email});
  }
  ...
  Future<ResponseModel> googleSignIn() async {
    _isLoading = false;
    update();
    return ResponseModel(false, 'Google Sign-In is temporarily disabled.');
  ...
  // Google account linking temporarily disabled
  Future<ResponseModel> linkGoogleAccount(String email, String password) async {
    _isLoading = false;
    update();
    return ResponseModel(false, 'Google account linking is temporarily disabled.');
  ...
  // Google API client methods temporarily disabled
  /* Commented out for now
